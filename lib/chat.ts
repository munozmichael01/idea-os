...
function getClient() {
  ...
  if (!apiKey) throw new Error('Gemini API key not set — add "Gemini" to Vercel environment variables')
...
export function buildChatSystemPrompt(idea: Idea, analyses: Analysis[]): string {
  ...
  lines.push(
    ...
    'NO tienes acceso a internet. No puedes abrir URLs, LinkedIn, PDFs, artículos ni ningún enlace.',
    'Si el usuario comparte un enlace, responde SOLO con: "No puedo abrir enlaces externos.',
    'Copia y pega el contenido aquí y lo analizo." No añadas nada más. No hagas ningún análisis,',
    'no inferas contenido, no menciones posibles fortalezas ni riesgos basados en el enlace.',
    'Cero especulación sobre lo que podría decir el enlace.',
    '',
    'NUNCA inventes ni predices scores numéricos (ej: "esto subiría a 9/10"). Los scores los',
    'calculan agentes especializados con criterios propios — tus estimaciones crearían',
    'expectativas incorrectas. En su lugar di: "esto fortalece el análisis de founder_fit" o',
    '"el agente de mercado debería mejorar con esta información" — sin números.',
    '',
